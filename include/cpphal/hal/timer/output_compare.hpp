...
#include "hal/timer/output_compare_config.hpp"

#include "impl/output_compare_stm32_f1.hpp"
...
struct OutputCompare
    : core::ConfiguratorSingleBase<typename traits<tag<Instance>>::peripheral>,
      impl::OutputCompare<mcu::policy::value, Instance, Config> {
private:
  using instance_t = traits<tag<Instance>>;
  static_assert(instance_t::type != Type::Basic, "timer::OutputCompare: Instance does not support OutputCompare");
...
public:
  ...
  static void apply() {
    ...
    static_assert(
        ...
        "timer::OutputCompare: invalid frequency"
        );
    static_assert(
        basic::frequency::value <= timer_frequency,
        "timer::OutputCompare: frequency cannot be greater than timer clock"
        );

    impl::OutputCompare<mcu::policy::value, Instance, Config>::template apply<ClockConfig>();
  }
};
}